...
// the real policy decider. It runs on the servant goroutine once the channel is
// up; Serve must be started after it returns, so the engine's signal
// round-trips can be answered.
//
...
func installEnforcement(
	...
	if policyDoc == nil {
		return // no policy: pure observe, no interceptor
	}
