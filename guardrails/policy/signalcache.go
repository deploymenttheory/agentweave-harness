...
func newSignalCache(reg *signals.Registry, policy *Policy, env func() *signals.Env) *signalCache {
	c := &signalCache{
		reg:     reg,
		env:     env,
		now:     time.Now,
	...
	for id, cfg := range policy.Signals {
		...
		c.entries[id] = cachedSignal{ttl: cfg.TTL.Std(), result: signals.Result{ID: id, Status: signals.Skip}}
	}
	return c
}

// argFor returns the configured argument for a signals.
...
// reporting signals.Skip is more honest than inventing a pass.
func (c *signalCache) Read(ctx context.Context, id string, args map[string]string) signals.Result {
	c.mu.Lock()
	entry, held := c.entries[id]
	now := c.now()
	if !held {
		c.mu.Unlock()
		return signals.Result{ID: id, Status: signals.Skip, Detail: "signal is not declared by the active policy"}
	}
...
func (c *signalCache) evaluate(ctx context.Context, id, arg string) signals.Result {
	g, ok := c.reg.Get(id)
	if !ok {
		return signals.Result{ID: id, Status: signals.Skip, Detail: "signal is not registered in this build"}
	}
	env := c.env()
	if env == nil {
		return signals.Result{ID: id, Status: signals.Error, Detail: "no evaluation environment available"}
	}
	e := *env
	e.Arg = arg
...
// Refresh re-evaluates every expired signals. It is registered as an in-flight
...
// severity the policy assigned to that signals.
...
func (c *signalCache) ReadAll(ctx context.Context, args map[string]string) []signals.Result {
	ids := c.ids()
	out := make([]signals.Result, 0, len(ids))
	for _, id := range ids {
		res := c.evaluate(ctx, id, args[id])
		c.mu.Lock()
		entry := c.entries[id]
...
// document. Entries never read are included with signals.Skip status so the reader can
// tell "not yet evaluated" from "evaluated and passing".
func (c *signalCache) Snapshot() []signals.Result {
	c.mu.Lock()
	defer c.mu.Unlock()
	out := make([]signals.Result, 0, len(c.entries))
