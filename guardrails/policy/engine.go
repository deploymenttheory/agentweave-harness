...
//
// It is an interface for the same reason signals.SystemProbe and signals.HealthProbe are: it
...
func (e *Engine) Signals() []signals.Result { return e.cache.Snapshot() }
...
func (e *Engine) ReadAll(ctx context.Context) []signals.Result { return e.cache.ReadAll(ctx, e.args) }
...
// DecisionFrom renders a verdict as the signals.Decision document the status surface and
// audit chain already understand, so the policy engine reuses one shape rather
// than inventing a second.
func (e *Engine) DecisionFrom(v Verdict, device signals.DeviceIdentity, rc signals.RunContext) signals.Decision {
	d := signals.Decision{
		Device:     device,
		RunContext: rc,
